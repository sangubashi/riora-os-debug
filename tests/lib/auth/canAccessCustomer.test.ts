...
// 2026-09-11: 担当制によるカルテ閲覧制限(旧Rule A'/B'/C、AUTH-1 V2)はユーザー承認により撤廃。
// canAccessCustomer() / filterAccessibleCustomerIds() は、内部ユーザー(is_internal_user=true)
// でなければ常にtrue/候補に含める。PHASE SECURITY-H1の内部ユーザー除外ゲートのみ従来どおり維持する。
//
// 本番の内部ユーザー2件(外舘裕子・鈴木雅子)はいずれも来店履歴を持ち、その最新来店担当スタッフが
// 同一人物(「外舘」)であることを本番DB調査で確認済みのため、TC群にもこの実データ形状
// (internal顧客でも旧Rule A'相当の一致条件が成立し得る)を再現し、それでも常にfalse/除外に
...
/** 実装が使うチェーン(eq/in/is/single、および終端メソッド無しでのawait)だけを
 * サポートする最小限のフルエントモック。 */
class QB {
  private rows: Row[]
...
describe('canAccessCustomer / filterAccessibleCustomerIds (担当制撤廃後・H-1維持)', () => {
  beforeEach(() => {
    resetDb()
  ...
  describe('canAccessCustomer', () => {
    it('非管理者 + is_internal_user=true は、旧Rule A\'が本来成立する場合でもfalse(外舘裕子相当)', async () => {
    ...
    it('非管理者 + is_internal_user=false + 担当が自分と一致 → true', async () => {
      const { canAccessCustomer } = await import('@/lib/auth/canAccessCustomer')
    ...
    it('非管理者 + is_internal_user=false + 担当が自分と不一致でもtrue(担当制撤廃)', async () => {
      const { canAccessCustomer } = await import('@/lib/auth/canAccessCustomer')
      ...
      await expect(canAccessCustomer('staff-someone-else', 'cust-none', false)).resolves.toBe(true)
    })

    it('非管理者 + is_internal_user=false + 来店・予約履歴なしでもtrue', async () => {
      const { canAccessCustomer } = await import('@/lib/auth/canAccessCustomer')
  ...
  describe('filterAccessibleCustomerIds', () => {
    it('非管理者: is_internal_user=trueの顧客は、旧Rule A\'が本来成立する担当者から見ても候補から除外される', async () => {
    ...
    it('非管理者: 担当の一致・不一致・履歴なしが混在していても、内部ユーザー以外は全て候補に含まれる(担当制撤廃)', async () => {
      const { filterAccessibleCustomerIds } = await import('@/lib/auth/canAccessCustomer')
      DB.brain_customers = [
      ...
      expect(result).toEqual(new Set(['cust-a', 'cust-c', 'cust-none']))
    })
