// ================================================================
// DELETE /api/customers/[id]/facial-schemas/[schemaId] ルートレベル受入テスト
//
// 実装計画(顔シェーマ機能READ ONLY設計・Phase 0、2026-09-21)Phase 3に対応:
//   - 未認証/認可なしを拒否する(401/403)
//   - 他顧客のschemaIdを指定した場合(所有権不一致)は403で拒否する(IDOR防止)
//   - 正常系は論理削除(deleted_at更新)のみ行い204を返す
//
// customer-photos-delete.test.ts と同じ方針・同じフェイクSupabaseの形を踏襲する。
// ================================================================
import { NextRequest } from 'next/server'
import { afterEach, describe, expect, it, vi } from 'vitest'
import type { RequestingStaff } from '../../src/lib/auth/extractStaffFromRequest'

vi.mock('../../src/lib/auth/extractStaffFromRequest', () => ({
  extractStaffFromRequest: vi.fn(),
}))
vi.mock('../../src/lib/auth/canAccessCustomer', () => ({
  canAccessCustomer: vi.fn(),
}))
vi.mock('../../src/lib/facialSchema/verifyFacialSchemaOwnership', () => ({
  verifyFacialSchemaOwnership: vi.fn(),
}))
vi.mock('../../src/lib/facialSchema/facialSchemaDb', () => ({
  getFacialSchemaServiceClient: vi.fn(),
}))

import { extractStaffFromRequest } from '../../src/lib/auth/extractStaffFromRequest'
import { canAccessCustomer } from '../../src/lib/auth/canAccessCustomer'
import { verifyFacialSchemaOwnership } from '../../src/lib/facialSchema/verifyFacialSchemaOwnership'
import { getFacialSchemaServiceClient } from '../../src/lib/facialSchema/facialSchemaDb'
import { DELETE } from '../../app/api/customers/[id]/facial-schemas/[schemaId]/route'

const mockExtractStaff = vi.mocked(extractStaffFromRequest)
const mockCanAccess    = vi.mocked(canAccessCustomer)
const mockVerifyOwner  = vi.mocked(verifyFacialSchemaOwnership)
const mockGetClient    = vi.mocked(getFacialSchemaServiceClient)

const STAFF: RequestingStaff = {
  authUserId: 'auth-user-1', staffBrainId: 'brain-staff-1', email: 'staff@example.com', isAdmin: false,
}

function fakeUpdateClient(updateSpy: (patch: unknown) => void) {
  const obj: Record<string, unknown> = {}
  obj.update = vi.fn((patch: unknown) => { updateSpy(patch); return obj })
  obj.eq = vi.fn(() => obj)
  obj.then = (resolve: (v: unknown) => unknown) => Promise.resolve({ error: null }).then(resolve)
  return { from: vi.fn(() => obj) }
}

function callRoute(customerId: string, schemaId: string) {
  const req = new NextRequest(`http://localhost/api/customers/${customerId}/facial-schemas/${schemaId}`, { method: 'DELETE' })
  return DELETE(req, { params: Promise.resolve({ id: customerId, schemaId }) })
}

afterEach(() => {
  vi.clearAllMocks()
})

describe('DELETE /api/customers/[id]/facial-schemas/[schemaId]', () => {
  it('未認証は401を返す', async () => {
    mockExtractStaff.mockResolvedValue(null)
    const res = await callRoute('customer-a', 'schema-1')
    expect(res.status).toBe(401)
    expect(mockVerifyOwner).not.toHaveBeenCalled()
  })

  it('アクセス権のない顧客は403を返す', async () => {
    mockExtractStaff.mockResolvedValue(STAFF)
    mockCanAccess.mockResolvedValue(false)
    const res = await callRoute('customer-forbidden', 'schema-1')
    expect(res.status).toBe(403)
    expect(mockVerifyOwner).not.toHaveBeenCalled()
  })

  it('他顧客のschemaId(所有権不一致)は403を返し、削除処理に進まない(IDOR防止)', async () => {
    mockExtractStaff.mockResolvedValue(STAFF)
    mockCanAccess.mockResolvedValue(true)
    mockVerifyOwner.mockResolvedValue(null)

    const res = await callRoute('customer-a', 'schema-of-other-customer')

    expect(res.status).toBe(403)
    expect(mockGetClient).not.toHaveBeenCalled()
  })

  it('正常系: 論理削除(deleted_at更新)のみを行い204を返す', async () => {
    mockExtractStaff.mockResolvedValue(STAFF)
    mockCanAccess.mockResolvedValue(true)
    mockVerifyOwner.mockResolvedValue({ id: 'schema-1', customerId: 'customer-a' })

    let capturedPatch: unknown = null
    mockGetClient.mockReturnValue(fakeUpdateClient(p => { capturedPatch = p }) as never)

    const res = await callRoute('customer-a', 'schema-1')

    expect(res.status).toBe(204)
    expect(capturedPatch).toHaveProperty('deleted_at')
    expect(typeof (capturedPatch as { deleted_at: string }).deleted_at).toBe('string')
  })

  it('DBエラー時は500を返す', async () => {
    mockExtractStaff.mockResolvedValue(STAFF)
    mockCanAccess.mockResolvedValue(true)
    mockVerifyOwner.mockResolvedValue({ id: 'schema-1', customerId: 'customer-a' })

    const obj: Record<string, unknown> = {}
    obj.update = vi.fn(() => obj)
    obj.eq = vi.fn(() => obj)
    obj.then = (resolve: (v: unknown) => unknown) => Promise.resolve({ error: { message: 'db down' } }).then(resolve)
    mockGetClient.mockReturnValue({ from: vi.fn(() => obj) } as never)

    const res = await callRoute('customer-a', 'schema-1')
    expect(res.status).toBe(500)
  })
})
