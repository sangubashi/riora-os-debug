-- ============================================================
-- ★★★ 2026-08-07時点で未適用と判明・実行しないこと ★★★
-- docs/STAFF_LOGS_SCHEMA_MISMATCH_DESIGN.md の調査で、本ファイルが本番へ
-- 未適用のまま放置されていたことが判明した。このDROP→再作成版は不採用と
-- なり、代わりに20260807000000_staff_logs_add_missing_columns.sql
-- (既存データを保持したままの追加専用ALTER TABLE、案A)を適用した。
-- 本ファイルは経緯の記録として残すのみで、今後実行してはならない
-- (実行すると既存データが失われ、20260807000000と内容が重複・矛盾する)。
-- ============================================================
--
