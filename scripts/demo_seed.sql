...
-- ─── 2. customers UPDATE (全30件をリアルなサロンデータへ) ─────────────────────────

-- STAFF_ID: ae68433d-69ce-4dc3-a38e-cc2501895fee
...
UPDATE customers SET
  ...
  updated_at=NOW()
WHERE id='7fc6d7c6-fd9b-48ba-9f4a-168d7151824c'; -- 田中 あかり
...
UPDATE customers SET
  ...
  updated_at=NOW()
WHERE id='e5219ca4-f2ac-44e9-9f8a-5b0633bd5a58'; -- 鈴木 美咲
...
UPDATE customers SET
  ...
  updated_at=NOW()
WHERE id='487e4f9f-223c-44a4-8484-8b04177da846'; -- 高橋 優太
...
UPDATE customers SET
  ...
  updated_at=NOW()
WHERE id='01a5030f-dc20-4a69-b3b4-7107da60b773'; -- 佐藤 花子
...
UPDATE customers SET
  ...
  updated_at=NOW()
WHERE id='f232275d-6ef0-4500-b561-3de1e6564ffd'; -- 伊藤 遥（今日来店）
...
UPDATE customers SET
  ...
  updated_at=NOW()
WHERE id='bcc30b15-04ec-4238-9c76-22dc4ad52bd1'; -- 佐々木 翔（3日前来店→無断キャンセル歴あり）

UPDATE customers SET
  customer_type='慎重・不安型', is_vip=false, visit_count=2, total_spent=25600,
  last_visit_date=CURRENT_DATE - INTERVAL '10 days',
  next_visit_date=NULL,
  churn_risk_score=10,
  memo='2回目来店。前回より打ち解けた様子。次回予約を促したが「また連絡します」で終了。',
  updated_at=NOW()
WHERE id='58a1b27e-10ae-4221-a005-5a606743f75f'; -- 中島 瞳

UPDATE customers SET
  customer_type='慎重・不安型', is_vip=false, visit_count=1, total_spent=12800,
  last_visit_date=CURRENT_DATE - INTERVAL '7 days',
  next_visit_date=NULL,
  churn_risk_score=8,
  memo='男性。初来店。彼女へのプレゼントに考えているとのこと。ギフト券の案内をした。',
  updated_at=NOW()
WHERE id='218bd5bd-cc0e-4c8d-9102-af1fe643befa'; -- 中村 健太
...
UPDATE customers SET
  ...
  updated_at=NOW()
WHERE id='a577818c-a35a-4270-bc18-b7d71813b747'; -- 森田 彩花

UPDATE customers SET
  customer_type='効果重視型', is_vip=false, visit_count=4, total_spent=123000,
  last_visit_date=CURRENT_DATE - INTERVAL '38 days',
  next_visit_date=NULL,
  churn_risk_score=50,
  memo='男性。効果重視型。前回施術後に「肌が変わった気がする」と言って帰った。継続してほしい。',
  updated_at=NOW()
WHERE id='ecaece03-bd76-429b-b5b2-40fc0f5a4264'; -- 石川 裕也

UPDATE customers SET
  customer_type='慎重・不安型', is_vip=false, visit_count=3, total_spent=91000,
  last_visit_date=CURRENT_DATE - INTERVAL '36 days',
  next_visit_date=NULL,
  churn_risk_score=48,
  memo='男性。花粉症で肌が荒れやすい春に来店が多い。来春の来店に向けて関係を維持したい。',
  updated_at=NOW()
WHERE id='17f8102c-ed71-4dd7-8e9a-db83290bd80c'; -- 清水 健
...
-- 無断キャンセル 45日前
('601f7ea0-2b33-4acc-9ab8-e8ca7a3f7f71','ae68433d-69ce-4dc3-a38e-cc2501895fee','ベーシックフェイシャル',12800,NOW()-INTERVAL '45 days',60,'no_show',false,'当日連絡なし。事前確認のLINEも既読スルー'),
-- 渡辺 恵美 5回
...
-- 無断キャンセル 50日前
('bd6df646-1e28-4b2b-a0ed-7077c3d6a216','ae68433d-69ce-4dc3-a38e-cc2501895fee','リラクゼーションコース',15000,NOW()-INTERVAL '50 days',75,'no_show',false,'3日前のリマインドLINEに反応なし'),
-- 小川 遥香 3回
...
-- ─── 6. kpi_today VIEW 確認用クエリ ─────────────────────────────────────────
-- （実行後、kpi_today に値が出るか確認）
...
UNION ALL SELECT 'no_show', count(*) FROM reservations WHERE status = 'no_show';
