'use client'
/**
 * InitialQuestionnaireCaptureModal.tsx — 顧客トップページ「初回問診票」の撮影・画像選択
 * モーダル(2026-09-24ユーザー承認)。
 *
 * IpadPhotoCaptureModal.tsx(iPadスタッフカルテの「前回|今回」比較用写真カルテ)とは
 * 意図的に別コンポーネントとして新設する。紙の問診票をスキャンするだけの単純な用途のため、
 * ゴースト重ね合わせ・顔検出ガイド・傾きガイド・ズーム表示・部位タブは実装しない
 * (IpadPhotoCaptureModal.tsx自体には一切手を加えない)。
 *
 * 撮影・変換ロジックは既存の低レベル関数(captureFrame.ts・fileToWebpBlob.ts)を
 * そのまま再利用し、保存のみ専用API(PUT /api/customers/[id]/initial-questionnaire、
 * brain_customers.initial_questionnaire_photo_path)へ行う。既存のbrain_customer_photos・
 * ghostSelection.ts・comparisonSelection.tsには一切関わらない。
 *
 * 画像メモリ解放: usePhotoCapture.tsと同じくpreview用object URLをURL.revokeObjectURL()で
 * 都度破棄する(iPadパフォーマンス対策)。
 */
...
export default function InitialQuestionnaireCaptureModal({ customerId, onClose, onSaved }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      form.set('file', pendingBlob, `questionnaire.${pendingBlob.type === 'image/jpeg' ? 'jpg' : 'webp'}`)
      const res = await authedFetch(`/api/customers/${customerId}/initial-questionnaire`, {
        method: 'PUT',
        body:   form,
      })
