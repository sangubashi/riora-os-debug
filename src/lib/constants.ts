...
/**
 * お客様モードからスタッフ用カルテへ切り替える際の4桁PIN(2026-09-22ユーザー承認)。
 *
 * 従来のロゴ1000ms長押し(PHASE IPAD-KARTE-ENTRY-1)に代えて採用。お客様が偶然/興味本位で
 * スタッフ画面へ迷い込むことを防ぐための「誤操作防止」目的であり、本格的な認証機構では
 * ない(このアプリのAPI認可は既存のセッション/JWTベースの仕組みがそのまま担っており、
 * このPINはUIの切り替えゲートに過ぎない)。DEMO_STORE_ID等と同じ「クライアントに含まれて
 * よい値」の方針(秘匿すべき資格情報ではないため)。
 *
 * 値は暫定(仮の4桁)。実運用前にサロン側で任意の値へ変更してよい(この1箇所を書き換えるだけ)。
 */
export const STAFF_MODE_SWITCH_PIN = '1234'
