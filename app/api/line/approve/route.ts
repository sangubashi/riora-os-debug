...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // ─── 対象キュー項目の取得(skip/approve共通・アクセス制御に必要) ───────
    ...
    // SECURITY: 担当外顧客のキュー操作(skip/approve共通)を防ぐ(既存のcanAccessCustomerルールに従う)。
    // customer_idが無いキュー項目(顧客削除等で紐付けが失われた行)は担当判定ができないため、
    // 既存のcanAccessCustomerが持つ「isAdmin=trueは常に許可」ルールをそのまま踏襲し、
    // admin以外はデフォルト拒否とする(新しい権限仕様は作らない)。skipのみ許可してapproveは
    // 拒否する、といった非対称な権限緩和は行わない。
